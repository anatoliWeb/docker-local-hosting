...
1. **Не комітьте `.env`** — файл містить чутливі дані (Basic Auth hash).
2. **Не комітьте приватні ключі** — файли `certs/*.pem`, `certs/*.key` додані в `.gitignore`.
3. **Не публікуйте Traefik Dashboard в інтернет** — він призначений лише для локальної мережі.
4. **Не копіюйте CA private key на клієнтські ПК** — копіюйте лише сертифікат CA (rootCA.pem).
5. **Не використовуйте це як production-рішення** — без додаткового hardening це небезпечно.
6. **Регулярно оновлюйте Docker-образи** — слідкуйте за security advisories.
